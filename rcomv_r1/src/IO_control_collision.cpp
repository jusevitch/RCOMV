    ...
    : nh_private_("~")
{
  ...
  nh_private_.param<double>("qi_x", qi.x, 5);
  nh_private_.param<double>("qi_y", qi.y, 0);
  nh_private_.param<double>("qi_theta", qi.theta, 1.57);
  nh_private_.param<double>("qf_x", qf.x, -5);
  nh_private_.param<double>("qf_y", qf.y, 0);
  nh_private_.param<double>("qf_theta", qf.theta, 4.71);
...
double findDifference(double init_psi, double goal_psi)
{
  ...
  err = (fabs(err) <= M_PI) ? err : sign * fmod(fabs(err) - 2 * M_PI, M_PI);
  return err;
}
...
control_cmd IO_control_collision::collision_avoid()
{
  ...
  { // Keeps the node from crashing before the list is populated
    ...
    if (!collision_states.empty()){
      ...
      psi_collision_sum.y = 0.0;
      psi_collision_sum.z = 0.0;
      geometry_msgs::Vector3 grad_vector;
      grad_vector.x = 0.0;
      grad_vector.y = 0.0;
      grad_vector.z = 0.0;
